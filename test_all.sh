...
start_and_test_llm_rag() {
    cd "$SCRIPT_DIR/llm-rag-demo" || return 1

    echo -e "${BLUE}🚀 Starting LLM RAG Demo...${NC}"
    ...
    while [ $attempt -lt $max_attempts ]; do
        if curl -s http://localhost:8001/health > /dev/null 2>&1; then
            echo -e "${GREEN}✅ LLM RAG Demo is ready${NC}"
            break
        fi
        attempt=$((attempt + 1))
        sleep 1
    ...
    if [ $attempt -eq $max_attempts ]; then
        echo -e "${RED}❌ LLM RAG Demo failed to start${NC}"
        docker-compose logs --tail=20
        cd "$SCRIPT_DIR" || return 1
        return 1
    fi

    echo -e "${GREEN}✅ LLM RAG Demo tests passed${NC}"
...
if [ "$CLEAN_MODE" = true ]; then
    ...
    echo ""
    cleanup_project "llm-rag-demo" "LLM RAG Demo"
    echo ""
...
# Test 1: LLM RAG Demo
echo "🤖 Testing LLM RAG Demo..."
cd "$SCRIPT_DIR/llm-rag-demo" || exit 1

if curl -s http://localhost:8001/health > /dev/null 2>&1; then
    if curl -s http://localhost:8001/health | grep -q "healthy"; then
        echo -e "${GREEN}✅ LLM RAG Demo is running${NC}"
        echo "   API: http://localhost:8001"
        echo "   Docs: http://localhost:8001/docs"
    else
        echo -e "${RED}❌ LLM RAG Demo health check failed${NC}"
    fi
else
    if [ "$AUTO_START" = true ] || [ "$RESTART_MODE" = true ]; then
        start_and_test_llm_rag
    else
        echo -e "${YELLOW}⚠️  LLM RAG Demo not running.${NC}"
        echo -e "   ${BLUE}Start it with:${NC} cd llm-rag-demo && docker-compose up -d"
...
# Test 4: System Design (documentation only)
echo "🏗️  Checking System Design documentation..."
cd "$SCRIPT_DIR/system-design-demo" || exit 1

if [ -f "README.md" ] && [ -f "high-load-api.md" ] && [ -f "etl-pipeline.md" ] && [ -f "llm-orchestration.md" ]; then
...
echo ""
echo "Summary:"
llm_rag_running=$(curl -s http://localhost:8001/health > /dev/null 2>&1 && echo "✅ Running" || echo "❌ Not running")
